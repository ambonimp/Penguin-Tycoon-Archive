...
coroutine.wrap(function()
	local n1 = 2
	while true do
		if string.match(Paths.Player:GetAttribute("Tool"), "Chainsaw") then
			local Char = Paths.Player.Character
			if Char and Char:FindFirstChild("HumanoidRootPart") then
				if Char:FindFirstChild("WoodcuttingAnimChainsaw") == nil then
					local new = Services.RStorage.Animations.Chainsaw:Clone()
					new.Name = "WoodcuttingAnimChainsaw"
					new.Parent = Char
