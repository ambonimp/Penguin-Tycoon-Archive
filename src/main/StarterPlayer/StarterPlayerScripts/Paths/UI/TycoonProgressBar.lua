...
local Popup = UI.Top.ProgressBarReward
local PopupSession
...
local function UpdateBar(Tween)
    ...
    local Size = UDim2.fromScale((Dividers.UIListLayout.Padding.Scale+DividerSize)*Unlocked-(DividerSize/2), 1)--UDim2.fromScale(Unlocked / (Unlockables+1), 1)
    if Tween then
...
function Progress()
    ...
    if Unlocked == Unlockables then
        local NextIsland = GetIncompleteIslandIndex()
        if NextIsland then
            LoadIsland(NextIsland)
        else
            TycoonCompleted()
        end
    else
...
for Object in pairs(Remotes.GetStat:InvokeServer("Tycoon")) do
    Unlocking[GetIslandIndex(Buttons[Object])].Unlocked += 1
end
