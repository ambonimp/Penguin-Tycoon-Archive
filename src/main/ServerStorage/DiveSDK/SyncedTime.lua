...
local function init()
	if not isInited then
		local ok, message = pcall(function()
			...
			local response = HttpService:RequestAsync({Url="https://google.com", Method = "HEAD"})
		...
		if not ok then
			warn("Cannot get time from google.com. Make sure that http requests are enabled!", message)
			originTime = os.time()
			responseTime = tick()
			responseDelay = 0
		end
