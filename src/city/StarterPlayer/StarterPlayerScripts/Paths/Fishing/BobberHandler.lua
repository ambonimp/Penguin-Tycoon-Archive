...
if workspace:FindFirstChild("Zones") and workspace.Zones:FindFirstChild("Fishing") then
	raycastParams.FilterDescendantsInstances = {workspace.Zones.Fishing.Part}
end
...
function BobberHandler.GetLocation()
	if fishingModule.LastUpdate.isAFKFishing then
		...
		local hit, position, normal, mat = workspace:FindPartOnRayWithIgnorelist(ray,{localPlayer.Character,workspace.Zones.Fishing.Part})
