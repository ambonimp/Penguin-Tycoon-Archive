...
function Income:GemLoop()
	while true do
		for i, Player in pairs(game.Players:GetPlayers()) do
			if Player:GetAttribute("Loaded") and Modules.PlayerData.sessionData[Player.Name] and Player:GetAttribute("Next5Gems") == nil and Modules.PlayerData.sessionData[Player.Name]["NextGemRewardSaved"] ~= "city" then
			...
			elseif Player:GetAttribute("Loaded") and Modules.PlayerData.sessionData[Player.Name] and Player:GetAttribute("Next5Gems") then
				if Player:GetAttribute("Next5Gems")-os.time() < 0 then
					Income:AddGems(Player, 5, "gemLoop")
